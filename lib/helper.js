...
moment.locale('br');
exports.Mod11 = (num, base = 9, r = 0) => {
    let fator = 2;
    const getFatorAndUpdate = (f) => {
        fator = (fator == base) ? 2 : ++fator;
        return f;
    };
    const soma = __.reduce((acum, char) => acum + parseInt(char) * getFatorAndUpdate(fator), 0)(_.reverse(_.split(num, '')));
    return (r == 0) ? (soma * 10) % 11 == 10 ? 0 : (soma * 10) % 11 : soma % 11;
};
exports.dvBarra = (barra) => {
    const resto2 = exports.Mod11(barra, 9, 1);
...
exports.BinaryRepresentationForBarcodeData = (barcodeData) => {
    ...
    const chuncked = _.chunk(_.split(exports.BarCodeEven(barcodeData), ''), 2);
...
exports.bmpLineForBarcodeData = (barcodeData) => {
    ...
    const bmpBuffer = new buffer_1.Buffer(bmpHeader + bmpData.join(''), 'binary');
